...
- 确认六个设置分类可进入；分别使用工具栏返回、系统返回键和边缘手势，按弹层 → 原分类 → 设置首页逐级返回。
  只有设置首页返回才关闭设置；搜索、主页、主题或倍速弹层关闭后保留原分类。
- 下载线程数和主题在进程重启后保留；从过滤列表返回隐私与过滤。
- 在 1.3 倍系统字体及横屏下检查文字换行；可用宽度达到 720dp 且已进入分类时显示双栏。
  两侧工具栏返回和系统返回均回到单栏分类列表；字体变化重建页面后再次检查逐级返回。
...
- 标准视频进入增强全屏后检查单击显隐、双击暂停/快进、左右纵向亮度/音量、横向拖动进度。
...
- YouTube 和自定义容器全屏只显示网页控件；暂停、后台恢复和重新全屏时不应出现两套控制层。
- 增强控制栏应在持续播放遥测期间自动隐藏；打开并取消投屏选择窗口后应保持全屏。
- 直播不可跳转、DRM、带字幕/清晰度选项的网站另行验证；不要用普通 MP4 通过代替这些结果。

1. 打开一个包含视频的页面并手动播放目标视频。
2. 确认网页右下角没有浏览器悬浮倍速或投屏按钮；从菜单依次切换 0.5×、1.5×、2× 和 3×，视频的
   `playbackRate` 应立即更新，重新加载媒体源后仍维持本页选择。
3. 从菜单或增强全屏控制栏进入投屏；没有候选时，菜单应显示未检测到可直接投送地址的说明。
4. 有候选时打开选择窗口，确认只把当前播放流标注为“正在播放”，其它候选仍可手选。
   重复打开同一个标准 MP4 页面后应仍能识别候选，暂停不应清空候选；签名参数保持完整。
