...
def main():
    ...
    try:
        ...
        allow_id='permission_allow_foreground_only_button'
        web_tap=ux.tap
        ux.adb('reverse','tcp:8875','tcp:8875')
        ux.launch(base+'capture-fixture.html?case='+key)
        web_tap('Capture photo');tap_id('permission_deny_button');ux.expect('No file selected');assert not events()
        record('Denied runtime camera permission returns to the unchanged form')
        web_tap('Capture photo');tap_id(allow_id);ux.expect('Shutter');back()
        ux.expect('No file selected');assert not events();record('Cancelling the system photo camera does not select a file')
        web_tap('Capture photo');tap_id('shutter_button');tap_id('done_button')
        ...
        web_tap('Capture video');tap_id('shutter_button');time.sleep(2);tap_id('shutter_button');tap_id('done_button')
        ...
        count=len(events());web_tap('Capture photo');ux.expect('Shutter')
