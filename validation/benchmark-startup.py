...
values=[]
for i in range(a.warmups + a.rounds):
 ...
 if i >= a.warmups: values.append(int(match[1]))
 time.sleep(1.5)
...
output={'serial':a.serial,'sdk':adb('shell','getprop','ro.build.version.sdk').strip(),
 'warmups':a.warmups,'launch_ms':values,'median_ms':statistics.median(values),'app_pss_kb':int(match[1]) if match else None}
