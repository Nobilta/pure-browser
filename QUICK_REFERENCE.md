...
最终文件：`PureBrowser-v0.1.0-release.apk`（arm64-v8a，Android 14+）。脚本会输出实际
...
6. 打开直播页面；检测到媒体后右下角出现投屏按钮，选择页中当前流显示“正在播放”。
7. 在同一局域网有 DLNA 设备时，选择设备并观察投送结果。
8. 在设置中分别选择系统 Download 与 SAF 自定义目录，修改下载线程数并重启确认保留。
9. 下载支持 Range 的文件，验证进度、后台通知、重试、重名编号；分别测试“仅删除记录”
   与“删除记录和文件”。
10. 测试文件上传、页面内查找、全屏视频和权限取消。
...
```bash
unzip -l PureBrowser-v0.1.0-release.apk | rg 'lib/|AndroidManifest.xml'
...
默认 native 模块是 `adblock`、`cache`、`url_utils`；legacy 模块需显式
