...
Pure 浏览器采用 Kotlin + Jetpack Compose + Android WebView，并在三个计算密集路径使用
Rust JNI：广告规则匹配、字节 LRU 缓存和 URL/搜索分类。平台能力（WebView 生命周期、
...
`PureBrowser-v0.1.0-release.apk`。当前 APK 大小和 hash 以脚本输出为准。
