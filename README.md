...
- **标签与书签**：标签分组、搜索、页内查找，支持导入和导出书签；登录弹窗保留来源页面以接收登录回调。
- **视频播放**：全屏倍速播放、亮度与音量手势、画中画、后台播放，以及 DLNA 投屏。
- **下载管理**：暂停、继续、断点续传与批量删除，支持选择保存目录；每次下载先确认文件信息，重复请求自动合并，重新下载保留旧文件，长按存图可就地确认保存副本。
- **加载与恢复**：等待网页响应时显示进度，加载失败后可在内置报错页重试。
- **沉浸全屏**：隐藏系统栏与浏览器工具栏，网页铺满全屏；保留浮动按钮呼出浏览器操作，退出立即生效，不重建页面。
- **离线扫码**：用相机扫描或从图片识别二维码，无需联网。
- **个性化界面**：自定义首页，地址栏可放在顶部或底部，支持简体中文、繁体中文和英文；设置可导出为文件并在其他设备导入，每组保存成功后才确认导入，缺少规则的已启用订阅随后自动下载。
- **隐私管理**：无痕浏览、按网站管理权限、清理浏览记录与网站数据；网站设置损坏时可确认重置并恢复保存，无痕数据清理完成后才恢复页面访问。
