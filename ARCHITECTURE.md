...
动画参数集中在 `BrowserMotion`，底部面板和全屏页由 `BrowserSheetWindow` 共用进入、换页和换容器逻辑。
地址栏建议放在 Activity 内，避免独立窗口打断输入法和编辑状态，顺序为链接、搜索项、在线联想、书签和历史。
在线联想由 `SearchSuggestionProvider` 按 HTTPS 模板获取，无联想地址的引擎直接不出联想、不回退其他引擎；
请求带去抖和过期丢弃，无痕每次会话使用独立缓存，响应按声明的字符集严格解码，无效编码不缓存。
链接扫描始终按实际消耗的文本推进，避免嵌套协议头造成重复扫描。
