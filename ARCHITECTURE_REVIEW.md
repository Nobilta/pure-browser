...
当前能力与行为边界见 [功能与行为边界](FEATURES.md)，实测结果与未覆盖范围见 [回归报告](EMULATOR_TEST_REPORT.md)。
...
- `BrowserSheetHost` 在整个菜单路径中保留一个 `BrowserSheetWindow`，路由 key 只替换窗口内内容。
  ...
  同为 120ms。两个容器共用同一实现，切换期间始终有不透明层压在遮罩之上，不会露出网页。
...
- WebView 回调校验实例和页面代次；导航取消旧权限、SSL、查找与媒体结果。弹窗使用新 WebView，
  ...
  `onTrimMemory` 的每一级压力也只回收一个，只有进程即将被回收的最后一级才清空。
