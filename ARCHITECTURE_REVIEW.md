...
2026-09-13，0.7.2。当前功能和安装包以 [README](README.md) 为准，实际检查结果见
...
- `BrowserSheetHost` 保存父级 UI 状态并释放父级窗口。菜单滚动状态置于 Dialog 外；安全边距
  ...
  路由外层显式使用 `key(route.key)`；设置使用全屏 Dialog，由同一个窗口持有界面和系统返回，退出时同步移除窗口与回调。
  网站表单和管理列表禁用弹层拖动及边界拉伸，避免滚动边界与弹层高度/inset 更新相互驱动。
