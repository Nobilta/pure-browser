...
更新：2026-09-10，适用于 0.5.1。实际模拟器和网站结果见 [回归报告](../EMULATOR_TEST_REPORT.md)。

## 控制权与回退

增强播放器沿用网页中的 HTML 视频元素和 WebView 解码，只替换安全可交接的全屏控制界面。
这样可以沿用网站的 Cookie、登录与媒体加载逻辑，避免额外启动另一套播放器。

| 场景 | 控件与手势归属 |
|---|---|
| 网页内视频、直播 | 网页播放器；有可识别媒体时右下角只显示投屏按钮 |
| 标准 `<video controls>` 自身进入全屏 | 网页确认交接后显示增强控件及手势 |
| 网站自定义容器、YouTube 全屏 | 保留网站控件，不叠加浏览器标题栏、播放栏或手势层 |
| 无法访问的跨域播放器、交接失败 | 保留或恢复网页控件，系统返回仍可退出 |
| 标准 Blob/MSE 视频 | 可以交接控件；Blob 地址本身不能用于 DLNA 直投 |

```mermaid
stateDiagram-v2
    [*] --> Web
    Web --> Handoff: 标准视频全屏且增强开关开启
    Handoff --> Enhanced: 隐藏原控件并收到当前目标的确认
    Handoff --> Web: 失败、目标变化或过期回执
    Enhanced --> Web: 切回网页、离开全屏或目标变化
    Enhanced --> Enhanced: 控制同一视频元素
```

接管条件同时要求视频自身拥有全屏、控件可交接、目标身份稳定；不以 MP4/Blob 类型代替控件归属检查。
网页侧先保存 `controls` 与元素原有标记，加入只匹配当前元素的隐藏样式，确认成功后原生侧才启用控件。
网站 CSP 阻止样式、目标更换、超时或失败时回退。退出先撤下原生触摸层，再恢复网页控件和标记。
交接代数和 frame/video 身份阻止旧回执重新开启已退出的控件。
...
YouTube 常使用 MSE、分离音视频及短期签名请求。保持网页播放器可操作比强行重绘控件更可靠；
没有候选只代表当前嗅探器没有可供直接投送的媒体地址，不等价于已判定 DRM。

JRS 等多来源直播页面使用相同策略：先由站点加载直播，再根据播放元素和请求关联当前候选；
自定义网站播放器继续拥有触摸层。实际连通性、直播链接及截图按测试时间记录在回归报告中，
本地夹具不代替实体电视或在线站点验收。
