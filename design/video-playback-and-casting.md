...
更新：2026-09-11，适用于 0.7.0。本次增强播放器验证见 [专项记录](enhanced-player-validation.md)，其他模拟器和网站结果见 [回归报告](../EMULATOR_TEST_REPORT.md)。

## 控制权与回退

增强播放器沿用网页中已加载的 HTML 视频元素和 WebView 解码，接管网页内及全屏控制界面。
网站有自己的控件或容器不再成为排除条件，也不再按域名排除 YouTube。
直接 HTTP(S) 视频、无扩展名签名地址和 Blob/MSE 使用同一套交接流程，保留 Cookie、登录与媒体加载逻辑。

| 场景 | 控件与手势归属 |
|---|---|
| 网页内已加载视频、直播 | 尝试用对齐原画面的内嵌控件接管；无可跳转范围时禁用进度 |
| 已加载的视频自身或网站容器进入全屏 | 增强开关开启时自动接管，无需 `controls` 属性 |
...
交接代数和 frame/video 身份阻止旧回执重新开启已退出的控件；暂停时固定当前目标，兄弟视频开始播放不会抢走控制。
网页自己的清晰度、弹幕、DOM 字幕等操作可通过“网页控件”切回；HTML 视频原生文本轨道仍由原视频处理。
没有标准视频元素、无法访问的 iframe 或无法隔离的布局仍需使用网站播放器，不能据网络候选宣称已经接管。
...
`inline-player.js` 在原视频上方放置独立 Shadow DOM，提供播放/暂停、进度、倍速、静音、全屏和恢复。
视频最小 160×90 CSS px，可见裁剪区域至少 100×64；不足 320px 宽或 160px 高时使用紧凑布局，隐藏时间与静音。
每个文档最多扫描 64 个视频、同时挂载 8 个控制层。标签隐藏、页面离屏、销毁或增强开关关闭时释放。

只选择与视频尺寸接近且没有正文/表单/其他媒体的容器；保留原布局，只隐藏视频路径外的控件分支。
滚动、缩放、viewport 变化和媒体脉冲触发对齐与检查；页面弹窗遮挡时恢复网页，遮挡离开后可再次接管。
删除样式、隔离失效或用户主动恢复后，该视频的当前来源不再自动重试；更换来源/节点可重新尝试。
进入原生全屏前先释放内嵌层；退出后恢复一层内嵌控件，不同时保留两种增强触摸层。
...
原生全屏保持黑色视频背景、白色操作图标与有对比度的进度强调色；按钮与进度触摸区域至少 48dp。
倍速和投屏选择使用与浏览器同主题的 MD3 弹层，取消不会离开全屏。
...
视频提供的有序来源线索；只标记能建立关联的来源，不把页面中的任意清单冒充当前直播。
...
- 请求完成前禁用重复发送；销毁与取消通过代数丢弃迟到的结果。失败不会把待连接设备标为已连接。
...
本地 HTML 视频和远端会话的控制入口与状态独立。已覆盖本地 SOAP/状态机测试，未使用实体电视验收；设备报告 PLAYING 也不等同于已经观察到实体屏幕画面。
